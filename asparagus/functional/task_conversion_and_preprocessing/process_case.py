...
from yucca.functional.preprocessing import analyze_label
...
def process_numpy_case(path, image_save_path, pkl_save_path, preprocessing_config, dtype=torch.float32):
    if os.path.isfile(image_save_path) and os.path.isfile(pkl_save_path):
        return
    try:
        image = np.load(path)
...
def preprocess_case_for_training_with_label(
    ...
    if final_target_size is not None:
        images = pad_case_to_size(case=images, size=final_target_size, label=label)

    image_properties["foreground_locations"], image_properties["label_cc_n"], image_properties["label_cc_sizes"] = (
        analyze_label(label=label, enable_connected_components_analysis=False, per_class=True)
    )
