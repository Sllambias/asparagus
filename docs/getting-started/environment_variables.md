...
ASPARAGUS_EVAL_BOX_CONFIGS=/PATH1/TO/ADDITIONAL/EVAL_BOX/CONFIG/DIR:PATH2/TO/ADDITIONAL/EVAL_BOX/CONFIG/DIR:PATH3...
ASPARAGUS_EVAL_BOX_ENV_CMD="source /PATH/TO/conda.sh && conda activate MY_ENV"
```
...
- `ASPARAGUS_EVAL_BOX_CONFIGS` points to all directories containing EvalBox configs. When `asp_eval_box_[run,prepare_data,collect_results] --config-name MyEvalBoxConfig` is called all the given `ASPARAGUS_EVAL_BOX_CONFIGS` directories are scanned for MyEvalBoxConfig.yaml.
-  `ASPARAGUS_EVAL_BOX_ENV_CMD` is a shell command injected into every dispatched scheduler job and executed on the compute node. It is required by `asp_eval_box_run` and `asp_eval_box_prepare_data` (not by `asp_eval_box_collect_results`). It runs before the actual `asp_finetune_[cls,reg,seg]` / `asp_process` command in order to set up the environment the job needs.
